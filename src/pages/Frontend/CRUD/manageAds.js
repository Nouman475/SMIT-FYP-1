...
const ManageAds = () => {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      setLoadingAds(true); // Start loading state
      try {
        // Create a query that only retrieves documents where createdBy matches the current user's UID
        const adsQuery = query(collection(db, "items"), where("createdBy", "==", user.uid));
        const adsSnapshot = await getDocs(adsQuery);
  
        // Map through the fetched documents and set state
        const adsList = adsSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        setAds(adsList);
      } catch (error) {
        console.error("Error fetching ads:", error);
      } finally {
        setLoadingAds(false); // Stop loading state
      }
    };
  
    if (user) { // Ensure currentUser is defined
      fetchAds();
    }
  }, [db, user]); // Add currentUser as a dependency
